...
static imgsensor_info_struct imgsensor_info = { 
    .sensor_id = SP2508_FRONT_SENSOR_ID,
...
static void set_dummy(void)
{
       ...
       //printk("dummyline = %d, dummypixels(not used) = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    printk("Enter framerate = %d, min framelength should enable %d \n", framerate,min_framelength_en);
    frame_length = imgsensor.pclk / framerate * 10 / imgsensor.line_length;
	//printk("(1)Enter imgsensor.frame_length = %d, imgsensor.min_frame_length = %d \n", imgsensor.frame_length,imgsensor.min_frame_length);
	
    spin_lock(&imgsensor_drv_lock);
		imgsensor.frame_length = (frame_length > imgsensor.min_frame_length) ? frame_length : imgsensor.min_frame_length; 
    	imgsensor.dummy_line = (imgsensor.frame_length > imgsensor_info.pre.framelength) ? (imgsensor.frame_length - imgsensor_info.pre.framelength) : 0;
	...
	//printk("(2)Enter imgsensor.frame_length = %d, imgsensor.min_frame_length = %d \n", imgsensor.frame_length,imgsensor.min_frame_length);
    set_dummy();
   #endif
}   /*  set_max_framerate  */
...
static void write_shutter(kal_uint16 shutter)
{
	...
	if (shutter < 7)
	   ...
	   shutter = 0xffff;
	//printk("D0311_V1(1)shutter =%d, imgsensor.framelength =%d,realtime_fps =%d,imgsensor.min_frame_length = %d,imgsensor.dummy_line = %d\n", 
	//				shutter,imgsensor.frame_length,realtime_fps,imgsensor.min_frame_length,imgsensor.dummy_line);
	...
	//printk("(2)shutter =%d, framelength =%d,realtime_fps =%d\n", shutter,imgsensor.frame_length,realtime_fps);
	set_max_framerate(realtime_fps,0);
    //printk("(3)shutter =%d, framelength =%d,realtime_fps =%d\n", shutter,imgsensor.frame_length,realtime_fps);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	  ...
	  kal_uint8  iReg;
   	 ...
   	 {   
               ...
               //iReg+= (gain%BASEGAIN)/(0x10/BASEGAIN);//error: division by zero
   	    	 ...
   	    	 else
   	    	 {
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			          printk("SP2508MIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xfd,0x01);
	write_cmos_sensor(0x3f, 0x03); //Set mirror and flip ,add by allenyao 
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate) 
{
    ...
    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            //imgsensor.min_frame_length = imgsensor.frame_length;
            imgsensor.min_frame_length = imgsensor_info.pre.framelength + imgsensor.dummy_line;
			spin_unlock(&imgsensor_drv_lock);  
			//printk("[set_max_framerate_by_scenario/pre] imgsensor.frame_length = %d, imgsensor.min_frame_length = %d,dummy_line = %d \n", imgsensor.frame_length,imgsensor.min_frame_length,imgsensor.dummy_line);
			//set_dummy();         
            break;          
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            if(framerate == 0)
            ...
            //imgsensor.min_frame_length = imgsensor.frame_length;
            imgsensor.min_frame_length = imgsensor_info.normal_video.framelength + imgsensor.dummy_line;
			spin_unlock(&imgsensor_drv_lock);    
			//printk("[set_max_framerate_by_scenario/video] imgsensor.frame_length = %d, imgsensor.min_frame_length = %d,dummy_line = %d \n", imgsensor.frame_length,imgsensor.min_frame_length,imgsensor.dummy_line);
            //set_dummy();       
            break;
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:			
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
		            ...
		            //imgsensor.min_frame_length = imgsensor.frame_length;
		            imgsensor.min_frame_length = imgsensor_info.cap1.framelength + imgsensor.dummy_line;
		            spin_unlock(&imgsensor_drv_lock);
					//printk("[set_max_framerate_by_scenario/cap1] imgsensor.frame_length = %d, imgsensor.min_frame_length = %d,dummy_line = %d \n", imgsensor.frame_length,imgsensor.min_frame_length,imgsensor.dummy_line);
            //set_dummy();       
            } else {
        		    if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
                ...
                spin_lock(&imgsensor_drv_lock);
		            ...
		            //imgsensor.min_frame_length = imgsensor.frame_length;
		            imgsensor.min_frame_length = imgsensor_info.cap.framelength + imgsensor.dummy_line;
		            spin_unlock(&imgsensor_drv_lock);
					//printk("[set_max_framerate_by_scenario/cap] imgsensor.frame_length = %d, imgsensor.min_frame_length = %d,dummy_line = %d \n", imgsensor.frame_length,imgsensor.min_frame_length,imgsensor.dummy_line);
            //set_dummy();       
            }
                //set_dummy();
            break;  
        case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
            ...
            imgsensor.min_frame_length = imgsensor.frame_length + imgsensor.dummy_line;
            spin_unlock(&imgsensor_drv_lock); 
            //set_dummy();          
            break;
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
