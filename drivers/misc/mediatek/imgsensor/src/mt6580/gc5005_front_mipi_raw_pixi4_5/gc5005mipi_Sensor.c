...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005MIPI_FRONT_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
//	gc5005_printf_otp();		// add by duanjinhui to printf otp buffer
	for(index=0;index<2;index++)
	{
		...
		case 0x01:
			...
			otp_info.is_back_camera = gc5005_read_otp(info_start_add + 2);
...
// golden data is not in otp, so disable this code.
	...
	LOG_INF("GC5005_OTP_INFO:is_back_camera=0x%02x\n",otp_info.is_back_camera);
...
static void gc5005_gcore_update_awb(void)
{
	...
	kal_uint16 rg_typical=0xa3, bg_typical=0xa7;	 
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            *sensor_id = return_sensor_id();
            // ID(read from sensor) = 0x5005, but define is 0x5007 in this file, so should add 2.
            (*sensor_id) += 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            // ID(read from sensor) = 0x5005, but define is 0x5006 in this file, so should add 1.
            sensor_id += 2;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
        	  printk("duanjinhui -> front SENSOR_FEATURE_CHECK_SENSOR_ID\n");
...
UINT32 GC5005_FRONT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
