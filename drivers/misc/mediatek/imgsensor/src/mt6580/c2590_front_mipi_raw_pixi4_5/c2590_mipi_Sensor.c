...
#include "c2590_mipi_Sensor.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "C2590_SensroDriver"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = C2590MIPI_FRONT_SENSOR_ID,        //C2590MIPI_SENSOR_ID record sensor id defined in Kd_imgsensor.h
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            printk("opDebug %s sensor id = 0x%x", __func__, *sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("opDebug get_imgsensor_id OK,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("opDebug get_imgsensor_id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 C2590_FRONT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
