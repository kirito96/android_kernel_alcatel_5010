...
//		(*otp_ptr).vcm_id = read_cmos_sensor(addr + 2);
	...
	printk("opDebug -> flag = 0x%02x, m_id = 0x%02x, l_id = 0x%02x, y = 0x%02x, m = 0x%02x, d = 0x%02x\n",
					(*otp_ptr).flag, (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id, (*otp_ptr).production_year,
	...
	if((*otp_ptr).module_integrator_id == 0x02) {
	  (*otp_ptr).flag |= 0x40;
	}
...
static int read_otp_cail(struct otp_struct *otp_ptr)
{
  ...
  mDELAY(5);
	...
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		tmp_rg_h = read_cmos_sensor(addr);				// R/G bits[9:2]
		tmp_bg_h = read_cmos_sensor(addr + 1);		// B/G bits[9:2]
		tmp = read_cmos_sensor(addr + 2);
		...
		(*otp_ptr).g_rg_ratio = (int)(0.5722*512.0+0.5);
		(*otp_ptr).g_bg_ratio = (int)(0.5800*512.0+0.5);
	} else {
		(*otp_ptr).rg_ratio = 0;
		(*otp_ptr).bg_ratio = 0;
	...
	printk("opDebug -> [ov5670 otp]rg_h = 0x%2x, rg_l = 0x%02x, bg_h = 0x%02x, bg_l = 0x%02x\n", 
						tmp_rg_h, tmp_rg_l, tmp_bg_h, tmp_bg_l);
	printk("opDebug -> [ov5670 otp]rg = 0x%04x, bg = 0x%04x ,(*otp_ptr).flag = 0x%02x\n", 
...
static void apply_otp(struct otp_struct *otp_ptr)
{
	...
	printk("opDebug af -> %s [ov5670 otp] rg = 0x%04x, bg = 0x%04x\n", __func__, (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
	// apply OTP WB Calibration
	if ((*otp_ptr).flag & 0x20) {
		rg = (*otp_ptr).rg_ratio;
		bg = (*otp_ptr).bg_ratio;
		g_rg = (*otp_ptr).g_rg_ratio;		// duanjinhui.wt 0.5722
		g_bg = (*otp_ptr).g_bg_ratio;		// duanjinhui.wt 0.5800
		...
		printk("opDebug af -> %s [ov5670 otp]rg=0x%04x, bg=0x%04x, R_gain=0x%04x, B_gain=0x%04x, G_gain=0x%04x\n",
					 __func__,rg, bg, R_gain, B_gain, G_gain);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				read_otp_id(&ov5670_otp_data);		// read otp id information
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				*sensor_id =0x5670;
				if (ov5670_otp_data.module_integrator_id == 0x02)
					return ERROR_NONE;
			}
	...
	if ((*sensor_id != imgsensor_info.sensor_id) || (ov5670_otp_data.module_integrator_id != 0x02)) {
...
static kal_uint32 open(void)
{
	...
	if(read_otp_cail(&ov5670_otp_data)==0xe0)	{
		printk("opDebug -> %s -> apply_otp successful af\n", __func__);
	   apply_otp(&ov5670_otp_data);
	}	else {
		printk("opDebug -> %s -> apply_otp fail af\n", __func__);
...
UINT32 OV5670_BACKAF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
