...
//add by allenyao
#define s5k5e8_otp_back_ff
static kal_uint16 read_cmos_sensor(kal_uint32 addr);
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
 #ifdef s5k5e8_otp_back_ff
 static unsigned int golded_rg=603;
 static unsigned int golded_bg=483;
 static int s5k5e8_otp_readmoduleid(void){
	...
	write_cmos_sensor(0x0a02, 0x0e);
	write_cmos_sensor(0x0a00, 0x01);
       if(0x01==read_cmos_sensor(0x0a05))
	   	start_reg=0x0a05;
	else if(0x01==read_cmos_sensor(0x0a15))
		start_reg=0x0a15;
	else if(0x01==read_cmos_sensor(0x0a25))
		start_reg=0x0a25;
...
static int s5k5e8_otp_cal_back_ff(void){
	unsigned int start_reg=0;
	//unsigned int flag=0;
	unsigned int curren_awbrg_high=0;
	unsigned int curren_awbrg_low=0;
	...
	module_id=read_cmos_sensor(start_reg+1);
	curren_awbrg_high=read_cmos_sensor(start_reg+8);
	curren_awbrg_low=read_cmos_sensor(start_reg+9);
	curren_awbbg_high=read_cmos_sensor(start_reg+10);
	curren_awbbg_low=read_cmos_sensor(start_reg+11);
	...
	goldenawbrg=golded_rg;
	goldenawbbg=golded_bg;
...
static void set_shutter(kal_uint16 shutter)
{
	...
	printk("shuter is 0x%x,allenyao\n",shutter);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {		
				ret=s5k5e8_otp_readmoduleid();
				if(ret==0x2){
					*sensor_id=S5K5E8YX_SENSOR_ID_BACK_FF;
...
UINT32 S5K5E8YX_BACK_FF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
