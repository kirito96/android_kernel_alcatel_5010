...
#define OV5670_OTP_CALI				// add by duanjinhui.wt to apply otp cali
...
//VGA 120fps
	...
	write_cmos_sensor(0x3820,0x96); //90
	write_cmos_sensor(0x3821,0xc0); //c6
...
// add by duanjinhui.wt to apply otp cali
...
static int read_otp_id(struct otp_struct *otp_ptr)
{
	...
	printk("opDebug -> otp_flag = 0x%02x", otp_flag);
	addr = 0;
	...
	printk("opDebug -> flag = 0x%02x, m_id = 0x%02x, l_id = 0x%02x, v_id = 0x%02x, d_ic = 0x%02x, y = 0x%02x, m = 0x%02x, d = 0x%02x\n",
					(*otp_ptr).flag, (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id, (*otp_ptr).vcm_id, (*otp_ptr).driver_ic, 
	...
	write_cmos_sensor(0x5002,(0x08)|(temp1&(~0x08)));
       printk("opDebug -> [ov5670 otp]otp_flag = %x ,module_integrator_id = %x, lens_id = %x ,vcm_id=%x\n",otp_flag, (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id,(*otp_ptr).vcm_id );
	if((*otp_ptr).module_integrator_id == 0x44)
		(*otp_ptr).flag |= 0x40;
...
static int read_otp_cail(struct otp_struct *otp_ptr)
{
	int otp_flag, addr, i;
	int tmp=0 ;
	int tmp_rg_h=0, tmp_rg_l=0;
	int tmp_bg_h=0, tmp_bg_l=0;
	//set 0x5002[3] to "0"
  //init
	write_cmos_sensor(0x0100,0x01);
	mDELAY(5);

  tmp=read_cmos_sensor(0x3D85);
  write_cmos_sensor(0x3D85,(0x06)|(tmp&(~0x06)));

  write_cmos_sensor(0x3D8C,0x73);
  write_cmos_sensor(0x3D8D,0xBF);

  mDELAY(5);
	tmp = read_cmos_sensor(0x5002);
	write_cmos_sensor(0x5002, (0x00 & 0x08) | (tmp & (~0x08)));
	// read OTP into buffer
	write_cmos_sensor(0x3d84, 0xC0);
	write_cmos_sensor(0x3d88, 0x70);	// OTP start address
	write_cmos_sensor(0x3d89, 0x10);
	...
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		tmp_rg_h = read_cmos_sensor(addr + 1);		// R/G bits[9:2]
		tmp_bg_h = read_cmos_sensor(addr + 2);		// B/G bits[9:2]
		tmp = read_cmos_sensor(addr + 4);
		tmp_rg_l = (tmp >> 6) & 0x03;							// R/G bits[1:0]
		tmp_bg_l = (tmp >> 4) & 0x03;							// B/G bits[1:0]
		(*otp_ptr).rg_ratio = (tmp_rg_h << 2) | tmp_rg_l;
		(*otp_ptr).bg_ratio = (tmp_bg_h << 2) | tmp_bg_l;
		printk("opDebug -> origin rg/bg = 0x%02x, 0x%02x, 0x%02x\n", tmp_rg_h, tmp_bg_h, tmp);
		tmp_rg_h = read_cmos_sensor(addr + 5);		// Golden R/G bits[9:2]
		tmp_bg_h = read_cmos_sensor(addr + 6);		// Golden B/G bits[9:2]
		tmp = read_cmos_sensor(addr + 8);
		tmp_rg_l = (tmp >> 6) & 0x03;							// Golden R/G bits[1:0]
		tmp_bg_l = (tmp >> 4) & 0x03;							// Golden B/G bits[1:0]
		(*otp_ptr).g_rg_ratio = (tmp_rg_h << 2) | tmp_rg_l;
		(*otp_ptr).g_bg_ratio = (tmp_bg_h << 2) | tmp_bg_l;
		printk("opDebug -> origin g_rg/g_bg = 0x%02x, 0x%02x, 0x%02x\n", tmp_rg_h, tmp_bg_h, tmp);
	} else {
		(*otp_ptr).rg_ratio = 0;
		(*otp_ptr).bg_ratio = 0;
	...
	printk("opDebug -> [ov5670 otp]rg_h = 0x%02x, rg_l = 0x%02x, bg_h = 0x%02x, bg_l = 0x%02x\n", 
						tmp_rg_h, tmp_rg_l, tmp_bg_h, tmp_bg_l);
	printk("opDebug -> [ov5670 otp]rg = 0x%04x, bg = 0x%04x, g_rg = 0x%04x, g_bg = 0x%04x, flag = 0x%02x\n", 
						(*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio, (*otp_ptr).g_rg_ratio, (*otp_ptr).g_bg_ratio, (*otp_ptr).flag);
	...
	//set 0x5002[3] to "1"
	tmp = read_cmos_sensor(0x5002);
	write_cmos_sensor(0x5002,(0x08)|(tmp&(~0x08)));
...
static void apply_otp(struct otp_struct *otp_ptr)
{
	...
	printk("opDebug -> [ov5670 otp]rg = 0x%04x, bg = 0x%04x\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
	// apply OTP WB Calibration
	if ((*otp_ptr).flag & 0x20) {
		rg = (*otp_ptr).rg_ratio;
		bg = (*otp_ptr).bg_ratio;
		g_rg = (*otp_ptr).g_rg_ratio;		// duanjinhui.wt
		g_bg = (*otp_ptr).g_bg_ratio;		// duanjinhui.wt

		printk("opDebug -> [ov5670 otp]g_rg = 0x%04x, g_bg = 0x%04x\n", g_rg, g_bg);
		//calculate G gain
		R_gain = (g_rg * 1000) / rg;
		B_gain = (g_bg * 1000) / bg;
		G_gain = 1000;
		...
		printk("opDebug -> [ov5670 otp]rg=0x%04x, bg=0x%04x, R_gain=0x%04x, B_gain=0x%04x, G_gain=0x%04x\n",
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				read_otp_id(&ov5670_otp_data);
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				*sensor_id =0X5671;
				if (ov5670_otp_data.module_integrator_id == 0x44) {
					printk("opDebug -> ov5670 main ff successful, id = 0x%04x", *sensor_id);
					return ERROR_NONE;
				}
			}
	...
	if ((*sensor_id != imgsensor_info.sensor_id) || (ov5670_otp_data.module_integrator_id != 0x44)) {
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	// add by duanjinhui.wt to apply otp
#ifdef OV5670_OTP_CALI
	if(read_otp_cail(&ov5670_otp_data)==0xe0)
	{
		printk("opDebug -> %s -> ov5670 main ff otp apply successful\n", __func__);
	   apply_otp(&ov5670_otp_data);
	}	else {
		printk("opDebug -> %s -> ov5670 main ff otp apply fail\n", __func__);
...
UINT32 OV5670_BACKFF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
