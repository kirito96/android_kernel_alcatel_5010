...
//add by allenyao
#define s5k5e8_otp_back_af
#define golden_rg 0x272
#define golden_bg 0x23E
static kal_uint16 read_cmos_sensor(kal_uint32 addr);
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
 #ifdef s5k5e8_otp_back_af
static int s5k5e8_otp_read_moduleid(void)
 {
	...
	write_cmos_sensor(0x0a00, 0x01);
       if(0x01==read_cmos_sensor(0x0a04))
	   	start_reg=0x0a04;
	else if(0x01==read_cmos_sensor(0x0a1b))
		start_reg=0x0a1b;
	//else if(0x01==read_cmos_sensor(0x0a33))
		//start_reg=0x0a24;
	printk("starg_reg is 0x%x,%s\n",start_reg,__func__);
	if(0==start_reg){
		printk("This module there is not otp date ,%s\n",__func__);
		return -1;
	}
	module_id=read_cmos_sensor(start_reg+1);
	write_cmos_sensor(0x0a00, 0x04);
	write_cmos_sensor(0x0a00, 0x00);
	printk("module_id is 0x%x,%s\n",module_id,__func__);
	printk("s5k5e8_otp_read_moduleid  is ok,%s\n",__func__);
	return module_id;
}
 
static int s5k5e8_otp_cal_back_af(void){
	unsigned int start_reg=0;
	//unsigned int flag=0;
	unsigned int curren_awbrg_high=0;
	unsigned int curren_awbrg_low=0;
	unsigned int curren_awbbg_high=0;
	unsigned int curren_awbbg_low=0;
	unsigned int golden_awbrg_high=0;
	unsigned int golden_awbrg_low=0;
	unsigned int golden_awbbg_high=0;
	unsigned int golden_awbbg_low=0;
	unsigned int awb_high=0;
	unsigned int curren_awbrg=0;
	unsigned int curren_awbbg=0;
	unsigned int goldenawbrg=0;
	unsigned int goldenawbbg=0;
	unsigned int checksum=0;
	//unsigned int sum=0;
	//unsigned int sum1=0;
	unsigned int r_gain=0;
	unsigned int b_gain=0;
	unsigned int g_gain=0;
	unsigned int base_gain=0;
	unsigned int module_id=0;
	write_cmos_sensor(0x0a00, 0x04);
	write_cmos_sensor(0x0a02, 0x04);
	write_cmos_sensor(0x0a00, 0x01);
	...
	module_id=read_cmos_sensor(start_reg+1);
	curren_awbrg_high=read_cmos_sensor(start_reg+10);
	curren_awbrg_low=read_cmos_sensor(start_reg+11);
	curren_awbbg_high=read_cmos_sensor(start_reg+12);
	curren_awbbg_low=read_cmos_sensor(start_reg+13);
	golden_awbrg_high=read_cmos_sensor(start_reg+17);
	golden_awbrg_low=read_cmos_sensor(start_reg+18);
	golden_awbbg_high=read_cmos_sensor(start_reg+19);
	golden_awbbg_low=read_cmos_sensor(start_reg+20);
	awb_high=read_cmos_sensor(start_reg+6);
	checksum=read_cmos_sensor(start_reg+7);
	write_cmos_sensor(0x0a00, 0x04);
	write_cmos_sensor(0x0a00, 0x00);
	printk("module_id is 0x%x,%s\n",module_id,__func__);
	#if 0
	sum=flag+awbrg_low+awbbg_low+goldenawbrg_low+goldenawbbg_low+awb_high;
	sum1=(sum%0xff)+1;
	printk("flag is 0x%x,%s\n",flag,__func__);
	printk("awbrg_low is 0x%x,%s\n",awbrg_low,__func__);
	printk("awbbg_low is 0x%x,%s\n",awbbg_low,__func__);
	printk("goldenawbrg_low is 0x%x,%s\n",goldenawbrg_low,__func__);
	printk("goldenawbbg_low is 0x%x,%s\n",goldenawbbg_low,__func__);
	printk("awb_high is 0x%x,%s\n",awb_high,__func__);
	printk("checksum is 0x%x,%s\n",checksum,__func__);
	printk("sum is 0x%x,%s\n",sum,__func__);
	printk("sum 0xff +1 is 0x%x,%s\n",sum1,__func__);
	if(checksum!=((sum%0xff)+1)){
		printk("checksum is 0x%x,not the same as 0x%x,%s\n",checksum,((sum%0xff)+1),__func__);
	}
	#endif
	curren_awbrg=((curren_awbrg_high)<<8)|curren_awbrg_low;
	curren_awbbg=((curren_awbbg_high)<<8)|curren_awbbg_low;
	goldenawbrg=(golden_awbrg_high<<8)|golden_awbrg_low;
	goldenawbbg=((golden_awbbg_high)<<8)|golden_awbbg_low;
	printk("curren_awbrg is 0x%x,%s\n",curren_awbrg,__func__);
	printk("curren_awbbg is 0x%x,%s\n",curren_awbbg,__func__);
	printk("goldenawbrg is 0x%x,%s\n",goldenawbrg,__func__);
	printk("goldenawbbg is 0x%x,%s\n",goldenawbbg,__func__);
	/*use module vendor test golden data*/
	goldenawbrg=golden_rg;
	goldenawbbg=golden_bg;
	printk("goldenawbrg is 0x%x,%s\n",goldenawbrg,__func__);
	printk("goldenawbbg is 0x%x,%s\n",goldenawbbg,__func__);
	
	r_gain=goldenawbrg*1000/curren_awbrg;
	b_gain=goldenawbbg*1000/curren_awbbg;
	g_gain=1000;
	base_gain=g_gain;
	if((g_gain<1000)||(b_gain<1000)){
		if(r_gain<b_gain)
			base_gain=r_gain;
		else
			base_gain=b_gain;
	}
	printk("base_gain is 0x%x,%s\n",base_gain,__func__);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				ID=s5k5e8_otp_read_moduleid();
				if(ID==0x0a){
					*sensor_id = imgsensor_info.sensor_id;
					return ERROR_NONE;
				}
				else{
					printk("Read sensor module id fail,module ID is 0x0a,but reture id: 0x%x\n",ID);
...
UINT32 S5K5E8YX_BACK_AF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
