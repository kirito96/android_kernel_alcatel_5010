...
#include "s5k5e8yxfrontmipiraw_Sensor.h"
...
//add by allenyao
#define s5k5e8_otp_front
static kal_uint16 read_cmos_sensor(kal_uint32 addr);
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para);
 #ifdef s5k5e8_otp_front
 static unsigned int golded_rg=577;
 static unsigned int golded_bg=488;
...
static int s5k5e8_otp_cal_front(void){
	unsigned int start_reg=0;
	//unsigned int flag=0;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(ret==0x2){
					*sensor_id=S5K5E8YX_SENSOR_ID_FRONT;
...
UINT32 S5K5E8YX_FRONT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
