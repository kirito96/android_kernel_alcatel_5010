...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005MIPI_BACKAF_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
/*
static void gc5005_printf_otp(void)
{
	int addr;
	kal_uint8 value;
	...
	{
		value = gc5005_read_otp(addr);
		LOG_INF("Reg[0x%02x] = 0x%02x\n", addr, value);
	}
}
*/
...
static void gc5005_gcore_read_otp_info(void)
{
	...
	kal_uint8 flag1_add,flag_golden_add;	
	kal_uint8 info_rom_start=0x01, wb_rom_start=0x0f, golden_rom_start=0x14;
	kal_uint8 info_width=7, wb_width=2; //golden_width=2;
...
//For GC
	otp_info.gc_flag = (flag0>>2)&0x03;
	LOG_INF("GC5005_OTP : otp_info.gc_flag = 0x%x \n",otp_info.gc_flag);

	if(otp_info.gc_flag==0x01) 
	{
		otp_info.Vald1=0xb0;//c0
		otp_info.Val21=0x08;
		otp_info.Val29=0x14;//15
...
//INFO&WB&Golden
	...
	flag1_add=0x00;
	flag_golden_add=0x13;
	info_rom_start=0x01;
	wb_rom_start=0x0f;
	golden_rom_start=0x14;

	flag1 = gc5005_read_otp(flag1_add);
//	if ((flag1 != 0) && ((flag1 & 0x03) == 0)) {
//		LOG_INF("error, sensor maybe is gc5005_back_af\n");
//	}
	flag_golden = gc5005_read_otp(flag_golden_add);
	LOG_INF("GC5005_OTP : flag1 = 0x%x , flag_golden = 0x%x\n",flag1,flag_golden);
//	gc5005_printf_otp();		// add by duanjinhui to printf otp buffer
	for(index=0;index<2;index++)
	{
		...
		case 0x00:
			...
			break;
		case 0x01:
			LOG_INF("GC5005_OTP_INFO group%d has INFO data !!\n", index + 1);
			info_start_add = info_rom_start + index * info_width;		// 0x01, 0x08
			otp_info.module_id = gc5005_read_otp(info_start_add);
			otp_info.lens_id = gc5005_read_otp(info_start_add + 1); 
			otp_info.driver_ic = gc5005_read_otp(info_start_add + 2);
		...
		case 0x00:
			...
			break;
		case 0x01:
			LOG_INF("GC5005_OTP_WB group%d has AWB data !!\n", index + 1);						
			wb_start_add = wb_rom_start + index * wb_width;	// 0x0f, 0x011
			...
			else
			{
				...
				LOG_INF("rg_gain = 0x%02x, bg_gain = 0x%02x\n", otp_info.rg_gain, otp_info.bg_gain);
			}
			break;
		case 0x02:
		case 0x03:	
...
// golden data is not in otp, so disable this code.
#if 0
		// read golden data
		...
		case 0x01:
			...
			otp_info.golden_rg= gc5005_read_otp(golden_start_add);
			otp_info.golden_bg = gc5005_read_otp(golden_start_add + 1); 
	...
	LOG_INF("GC5005_OTP_INFO:module_id=0x%02x\n",otp_info.module_id);
	LOG_INF("GC5005_OTP_INFO:lens_id=0x%02x\n",otp_info.lens_id);
	LOG_INF("GC5005_OTP_INFO:vcm_id=0x%02x\n",otp_info.vcm_id);
	LOG_INF("GC5005_OTP_INFO:driver_ic=0x%02x\n",otp_info.driver_ic);
	LOG_INF("GC5005_OTP_INFO:data=%d-%d-%d\n",otp_info.year,otp_info.month,otp_info.day);
	LOG_INF("GC5005_OTP_WB:r/g=0x%02x\n",otp_info.rg_gain);
	LOG_INF("GC5005_OTP_WB:b/g=0x%02x\n",otp_info.bg_gain);
}
...
static void gc5005_gcore_update_awb(void)
{
	...
	kal_uint16 rg_typical=0x9a,bg_typical=0x9c;

// golden data is not in otp, so disable this code.
#if 0
	if(0x01==(otp_info.golden_flag&0x01))
	{
		LOG_INF("GC5005_OTP_UPDATE_AWB:get typical value from otp\n");
	...
	LOG_INF("GC5005_OTP_UPDATE_AWB:rg_typical = 0x%02x, bg_typical = 0x%02x, rg_gain = 0x%02x, bg_gain = 0x%02x\n", 
			rg_typical, bg_typical, otp_info.rg_gain, otp_info.bg_gain);
	r_gain_current = 0x80 * rg_typical/otp_info.rg_gain;
	b_gain_current = 0x80 * bg_typical/otp_info.bg_gain;
	g_gain_current = 0x80;
	
	// find the min gain as base_gain
	...
	LOG_INF("GC5005_OTP_UPDATE_AWB:r_gain_current = 0x%02x , b_gain_current = 0x%02x , base_gain = 0x%02x \n",r_gain_current,b_gain_current,base_gain);
	
	r_gain = 0x80 * r_gain_current / base_gain;
	g_gain = 0x80 * g_gain_current / base_gain;
	b_gain = 0x80 * b_gain_current / base_gain;
	LOG_INF("GC5005_OTP_UPDATE_AWB:r_gain = 0x%02x , g_gain = 0x%02x , b_gain = 0x%02x \n",r_gain,g_gain,b_gain);
...
{	
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		write_cmos_sensor(0xfe, 0x00);          
		write_cmos_sensor(0x21, otp_info.Val21);//0xd1=a0,21=0a;0xd1=c0,21=08
		write_cmos_sensor(0x29, otp_info.Val29);
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0x29, 0x22);//16
		write_cmos_sensor(0xd8, 0x04);
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0x29, 0x28);//19
	...
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xfe, 0x00);  
		write_cmos_sensor(0x21, 0x16);//10
		write_cmos_sensor(0x29, 0x28);//1b
		write_cmos_sensor(0xd8, 0x12);//08
		write_cmos_sensor(0xe8, 0x01);//00
		write_cmos_sensor(0xec, 0x01);//00
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xd1, otp_info.Vald1);                                                                                                         
...
#endif
				LOG_INF("module_id = 0x%02x, lens_id = 0x%02x, driver_ic = 0x%02x\n", otp_info.module_id, otp_info.lens_id, otp_info.driver_ic);
                if (otp_info.module_id != 0x0a) {
                    continue;
                }
//                LOG_INF("sensor_module id = 0x%x\n", otp_info.module_id);
                LOG_INF("find_gc5005_back_af i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
    ...
    if ((*sensor_id != imgsensor_info.sensor_id) || (otp_info.module_id != 0x0a)) {

        // if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF
        LOG_INF("get_imgsensor_id: Read sensor id fail, exit\n");
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id) {
    	  LOG_INF("is open function, Read sensor id fail, exit\n");
        return ERROR_SENSOR_CONNECT_FAIL;
		}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
        	  printk("duanjinhui -> backaf SENSOR_FEATURE_CHECK_SENSOR_ID\n");
...
UINT32 GC5005_BACKAF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
