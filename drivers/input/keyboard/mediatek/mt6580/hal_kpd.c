...
void long_press_reboot_function_setting(void)
{
	...
	//begin-20160111-yuduan.xie-add-for-mini-disable reboot
        /*20160119 by wuyuelei for task 1463297
        change [#ifdef TARGET_BUILD_MMITEST] to [#if defined ...]*/
	...
	#else
	//end-20160111-yuduan.xie-add-for-mini-disable reboot
		...
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_TD, CONFIG_KPD_PMIC_LPRST_TD);
	#endif
	#endif//begin-20160111-yuduan.xie-add-for-mini-disable reboot
#else
		kpd_info("disable normal mode LPRST\n");
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x00);
